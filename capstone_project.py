...
with header:
    ...
    if arr and dep and hotel and snap != None:
        ...
        df_arr_month = df_arr_filter.groupby(
            ["Month", "Hour_of_day"], as_index=False
        ).sum()
        ...
        if choice_2 == "Separate":
            ...
            fig_month.update_traces(texttemplate="%{text:.3s}", textposition="outside")
            fig_month.update_layout(uniformtext_minsize=10)
            st.plotly_chart(fig_month)
        else:
            df_arr_month = df_arr_filter.groupby(["Month"], as_index=False).sum()
            fig_month = px.bar(
                df_arr_month,
                x="Month",
                y="Counter",
        ...
        st.write(
            """<h3 style:'text-align:center'>The number of arrivals each hour by day or by month categorised by trip length of less than 5 days and 5 days or more.""",
            unsafe_allow_html=True,
        )
        df_merge = load_dataset_dep(df_arr, df_dep)
        st.write(f"Total of {df_merge['Counter'].sum()} passenger in this dataset")
        choice_3 = ["By Month", "By Day"]
        selection = st.radio("Change the layout of the chart", choice_3, key="3")
        ...
        if selection == "By Month":
            ...
            df_merge = df_merge.groupby(["Day", "Hour"], as_index=False).sum()
            fig_day_stay = px.bar(
                df_merge,
                x="Hour",
                y=["less_5", "more_equal_5"],
